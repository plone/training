...
This example will stay very simple,
we will use `jQuery <https://jquery.com/>`_
to simply modify the content of an element:
...
For more details on how to write a mockup pattern,
...
In our example, we are using the RequireJS ``define`` function
to define our pattern as a JavaScript module.


Integrating With LESS
=====================

Add a ``pattern.less`` file to the ``exercise5/static`` directory
and provide whatever styles you like for your pattern
...
Our bundle will use the ``require`` function
to include the JavaScript module pattern we created previously.
...
The only thing we are doing in this file
is including the ``exercise5`` module we defined earlier--that's it.
...
They do the dependency resolution and include the modules
that were required with them.
...
..  warning::
    ...
    If you are not running a ZEO setup,
    you will need to shut down your Plone instance
