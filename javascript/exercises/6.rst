...
that checks the minimum size of an image before it is uploaded.
...
This pattern simply has ``minWidth`` and ``minHeight`` options
and, when a file is selected for upload, will check to make sure it is a valid size.
...
..  warning::
    ...
    To make sure your resource registry configuration changes apply,
    you will need to be in development mode.
    You can also toggle development mode on and off, click save, to force configuration to be re-built after changes
